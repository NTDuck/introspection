...
int main(int argc, char* args[]) {
    ...
    flags.renderer = SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC;
    flags.image = IMG_INIT_PNG;
    flags.hints[SDL_HINT_RENDER_SCALE_QUALITY] = "1";

    Dat2u pos {SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED};
    Dat2hu size {1280, 720};

    unsigned short int frameRate = 120;
    std::string title = "8964";

    // actual instantiation
    Game game(flags, pos, size, frameRate, title);
    game.start();
