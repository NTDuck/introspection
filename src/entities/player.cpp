...
void Player::handleKeyboardEvent(const SDL_Event& event, TileCollection& tileCollection) {
    ...
    if (event.key.repeat || event.key.type != SDL_KEYDOWN || nextDestCoords != nullptr) return;   // Apparently `event.key.repeat` has to be there else the program magically terminates itself.
    ...
    if (!Player::validateMove(*nextDestCoords, tileCollection)) moveCleanup();
}
...
bool Player::validateMove(SDL_Point& nextDestCoords, const TileCollection& tileCollection) {
    ...
    if (nextDestCoords.x < 0 || nextDestCoords.y < 0 || nextDestCoords.x >= globals::TILE_DEST_COUNT.x || nextDestCoords.y >= globals::TILE_DEST_COUNT.y) {
        nextDestCoords.x -= velocity.x;
        nextDestCoords.y -= velocity.y;
        moveCleanup();
        return false;
